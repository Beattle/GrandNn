...
?>
<!-- Операции -->
<!-- <IMG SRC='<?=BAN::inc_tpl('a_oper.gif', $tmp)?>' BORDER='0' ALT='Опции' align="left" hspace=3 vspace=2 name="adm_im<?=$num?>" style="padding:0;margin:0" OnClick="writeField('EventsCmdLeft', getField('db_adm_menu_each<?=$num?>_l'));writeField('EventsCmdRight', getField('db_adm_menu_each<?=$num?>_r'));"> -->
<div id="db_adm_menu_each<?=$num?>_l" style="position:absolute;visibility:hidden">

<?if (!in_array($Cmd, $block)):?>
<!--Редактировать -->
<?/*<A HREF='javascript:;' onclick='ShowPopUp("/clear/<?=$_BAN['MODULE_NAME']?>/<?=$_BAN['KUR_ALIAS']?>/<?=$data['alias']?>?edit", 760, 500, "yes");'><IMG SRC='<?=BAN::inc_link('a_edit.gif', $tmp)?>' BORDER='0'  align='left' ALT='редактировать'>Редактировать</A>*/?>
<A HREF='javascript:;' onclick='if(shift_pressed_on==-1){shift_pressed_on=CurrentMarked;isp=true;}EnterPressed();if(isp)shift_pressed_on=-1;return false;'><IMG SRC='<?=BAN::inc_link('a_edit.gif', $tmp)?>' BORDER='0'  align='left' ALT='редактировать'>Редактировать</A>
<?endif;?>
</div>
...
<?if (!in_array($Cmd, $block)):?>
<!--Удалить-->
<?/* <A HREF='/<?=$_BAN['MODULE_NAME']?>/<?=$_BAN['KUR_ALIAS']?>/<?=$data['alias']?>?del' OnClick="return doc_del('<?=str_replace('\'',"\"",@$data['name'])?>', '/<?=$_BAN['MODULE_NAME']?>/<?=$_BAN['KUR_ALIAS']?>/<?=$data['alias']?>?del');"><IMG SRC='<?=BAN::inc_link('a_delete.gif', $tmp)?>'  BORDER='0'  align='left' ALT='удалить'>Удалить</A>*/?>
 <A HREF='/<?=$_BAN['MODULE_NAME']?>/<?=$_BAN['KUR_ALIAS']?>/<?=$data['alias']?>?del' OnClick="DeletePressed();"><IMG SRC='<?=BAN::inc_link('a_delete.gif', $tmp)?>'  BORDER='0'  align='left' ALT='удалить'>Удалить</A>
<?endif;?>
</div>
...
<?if (empty($own_name)){?>
	     ...
	     > <IMG SRC="<?=BAN::inc_link('a_oper.gif', $tmp)?>" BORDER='0' ALT='Опции' align="left"> <?=$data["name"]?></a></p>
<?}?>