<? 
	// подключаем структуру таблицы хранения заказов
	include $_CORE->ROOT."/modules/auth/land_data/auth_pers.dat";
	$dataIn = $data;
	$form = array();
	foreach($dataIn as $field => $value){
		if(isset($DATA['author_'.$field]) && !is_array($value)){
			$form['author_'.$field] = mysql_real_escape_string($value);
		}
		if($field == 'contacts'){
			foreach($value as $cField => $cValue ){
				if(isset($DATA['author_'.$cField]) ){
					$form['author_'.$cField] = mysql_real_escape_string($cValue);
				}
			}
		}
	}

$form['author_login'] = $data['email'];
$form['author_comment'] = $data['user_name'];

if ($form['author_login'] == 'admin') exit;

$userid = SQL::getval("author_id", DB_TABLE_PREFIX . "auth_pers", "author_login = '".$form['author_login']."'");

if( empty($userid) ) {
	include $_CORE->ROOT."/modules/auth/land_data/auth_pers.dat";
	$form['author_passwd']	= rand(100000,999999);
	$form['author_passwd']	= ($_CONF['PWDCRYPTED']) ? crypt($form['author_passwd'],SALT) : $form['author_passwd'];
	...
	$_SESSION['SESS_AUTH']['NAME']		= $auth['author_comment'];
...
if(!empty($_SESSION['SESS_AUTH']['ID']) && $_SESSION['SESS_AUTH']['ID'] == $userid ){
	...
	if(empty($_SESSION['SESS_AUTH']['ALL']['alias'])){
		$arrset[] = "alias = '".$userid."'";
	}
	if(count($arrset)){
...
SQL::upd(DB_TABLE_PREFIX.'basket', " from_auth = '$userid' ", "email = '".$form['author_login']."'");
