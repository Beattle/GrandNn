// htmlArea v3.0 - Copyright (c) 2002-2004 interactivetools.com, inc., dynarch.com, dynarch.com
...
<div class='title'\
 style='background: url(" + dialog.editor.imgURL("table-prop.gif", "TableOperations") + ") 98% 50% no-repeat'>" + i18n["Table Properties"] + "\
</div> \
...
"        <tr> \
  ...
  <tr> \
    <td> \
      <fieldset><legend>" + i18n["Frames and Borders"] + "</legend> \
        <table width='100%'> \
          <tr> \
...
TableOperations.prototype.dialogRowCellProperties = function(cell) {
	...
	function (dialog) {
		...
		dialog.content.innerHTML = " \
<div class='title'\
 style='background: url("+ dialog.editor.imgURL(cell ? "cell-prop.gif" : "row-prop.gif", "TableOperations") + ") 98% 50% no-repeat'>" + i18n[cell ? "Cell Properties" : "Row Properties"] + "</div> \
<table style='width:100%'> \
  <tr> \
    <td id='--HA-layout'> \
...
TableOperations.prototype.buttonPress = function(editor, button_id) {
	...
	switch (button_id) {
		...
		if (!HTMLArea.is_ie) {
			...
			var td = this.getClosest("td");
			if (!td) {
				alert(i18n["Bitte wählen Sie eine Zelle aus"]);
				break;
			}
...
TableOperations.createColorButton = function(doc, editor, color, name) {
	...
	span.onclick = function() {
		...
		editor._popupDialog("select_color_" + _editor_lang + ".html", function(color) {
			if (color) {
				span.style.backgroundColor = "#" + color;
				field.value = "#" + color;
			}
